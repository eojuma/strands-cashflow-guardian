...
- **Duplicate proposals** — the write path is idempotent too: `orchestrator.persist_proposed_actions()` skips a proposal when an equivalent action is already `pending` for the same `(client_id, action_type, reference)` (reference = `invoice_id`, `milestone_id`, or drafted content). Rejected/executed actions do not block a fresh proposal, so repeated scheduled runs can't grow the approvals queue.
